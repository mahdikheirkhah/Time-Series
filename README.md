...
- To calculate rolling and cumulative values for time series.
- To build a backtest.

### Learning Objectives

- **Manipulate** time series data using Pandas datetime indexing and resampling
- **Calculate** rolling averages, returns, and aggregated metrics on financial data
- **Transform** multi-asset price data into returns using vectorized operations
- **Implement** backtesting strategies to evaluate trading performance
- **Analyze** financial time series using candlestick charts and visualization tools
...
- `data/` folders - AAPL.csv dataset for exercises 2 and 4
...
We suggest using the most recent.
...
The data we will use is Apple stock from [`AAPL.csv`](./data/AAPL.csv).
...
1. **Without using a for loop**, compute the daily returns (return(d) = (price(d)-price(d-1))/price(d-1)) for all the companies and return a DataFrame as:
...
Note: The data is generated randomly, the values you have may lead to a different result. The example above shows the expected DataFrame structure.
...
1. **Use vectorized operations:** Avoid for loops in Pandas - use `.shift()`, `.pct_change()`, and `.rolling()` for time series calculations.

2. **Set datetime index early:** Convert date columns to datetime and set as index immediately for easier time series manipulation.

3. **Understand aggregation logic:** Choose aggregation functions based on data meaning - mean for prices, sum for volumes, min/max for highs/lows.

4. **Check data before backtesting:** Always verify data types, missing values, and date ordering before running backtests to avoid incorrect results.

5. **Visualize time series:** Use candlestick charts and line plots to understand price movements and validate calculations visually.

### Resources

- [Python Data Science Handbook](https://jakevdp.github.io/PythonDataScienceHandbook/) - Comprehensive guide covering time series analysis with Pandas
- [Pandas Cheat Sheet](https://pandas.pydata.org/Pandas_Cheat_Sheet.pdf) - Quick reference for Pandas operations
- [Complete Pandas Tutorial for Beginners](https://www.learndatasci.com/tutorials/python-pandas-tutorial-complete-introduction-for-beginners/) - Step-by-step introduction to Pandas
- [Backtesting Explained](https://www.investopedia.com/terms/b/backtesting.asp) - Introduction to backtesting trading strategies
- [Pandas Time Series Documentation](https://pandas.pydata.org/docs/user_guide/timeseries.html) - Official guide to working with dates and times
- [Plotly Candlestick Charts](https://plotly.com/python/candlestick-charts/) - Guide to creating financial charts

- Datafile [`AAPL.csv`](data/AAPL.csv) - Dataset

### AI Prompts For Learning

1. "Explain the difference between .shift(), .pct_change(), and .diff() in Pandas for time series analysis. Provide examples showing when to use each method for calculating returns and changes."

2. "How do you properly backtest a trading strategy in Python? Explain the key considerations including look-ahead bias, transaction costs, and slippage. Show code for a simple moving average crossover strategy backtest."
